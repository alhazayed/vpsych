...
**Date:** 2026-08-09  
**Amended:** 2026-08-10 — Phase 3A contract amendment (docs only; see `docs/VPsych_PHASE3A_CONTRACT_AMENDMENT.md`)  
**Branch:** `cursor/vp-creation-contract-b432` (original); amendment on `cursor/phase3a-contract-amendment-5e36`  
...
### 14.5 Draft vs Publish — runtime safety vs authored quality

| State | Representation | Human personality | Other incompleteness |
|---|---|---|---|
| **DRAFT** | `is_active = false` | May be incomplete; **may rely on** `synthesizeHumanPersonalityFromAvatar()` at runtime | Allowed under admin-only visibility |
| **PUBLISH** | `is_active = true` only after gates | Must satisfy authored-quality requirements defined by the publish contract (typically both locales passing `validateHumanPersonality`) | Dual native personalities, clinical_core richness, voice/disorder policy per gates |
...
3. **New slugs lack builtin HP catalogs and therapy-cue harvests** — runtime still synthesizes a valid Module 2b profile via `synthesizeHumanPersonalityFromAvatar()`, but published patients that skip **authored** HP risk generic / less distinctive characterization and miss Maya/Jordan cue shortcuts. Prefer authored HP for ACTIVE patients (quality gate, not Module 2b absence).  
...
3. Preferred bilingual voice model (one profile + legacy dual ids vs two profile FKs) — only one FK exists today; clinical delivery columns already exist on `voice_profiles` and must be reused, not redesigned.  
...
Docs-only correction after Phase 3B pre-flight FAIL. Corrected human-personality runtime (§§6.3, 9.2, 11.2, Risk #3), voice_profiles clinical inventory (§7), disorder clinical-code constraint (§§5, 8.4), `updated_at` sync behavior (§2.1), and Phase 2 wizard shell naming (§§4, 19). **No application code, schema, migrations, APIs, or Admin UX implementation changed.** See `docs/VPsych_PHASE3A_CONTRACT_AMENDMENT.md`.

---

*End of Phase 3A read-only contract (as amended). Do not implement create/publish APIs until this document is approved and Phase 3B is explicitly authorized.*
