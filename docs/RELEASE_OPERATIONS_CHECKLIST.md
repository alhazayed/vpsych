...
**RC3-C2 status after RDL-011:** Credential Verification Gate **PASS** (`docs/rc3/RC3_C2_GATE_PASS.md`). Auth + Vault are synchronized; browser + password-grant + role separation verified on production. Remaining before Wave 1: Release Manager updates **persistent** Cursor secrets from Vault (unswap emails + vault passwords), runs `node scripts/rc3-credential-gate-preflight.mjs`, then launches a **fresh** agent.
