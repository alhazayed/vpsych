...
> **Execution status:** Phases 0–3d completed 2026-08-05T12:45Z — see `docs/rc3/RC3_C2_GATE_PASS.md` (evidence `RC3-C2-EV-20260805T1245Z`, RDL-011). Auth passwords were already in Vault at 12:08Z; this run proved token grant, role separation, and browser login. Remaining: Release Manager must update **persistent** Cursor secrets from Vault before Wave 1.
>
> **Canonical audit emails (public Auth identities):** `audit.therapist@…` · `audit.admin@…`. Passwords stay in Vault only (`vpsych_audit_therapist_password` / `vpsych_audit_admin_password`).
...
|---|---|---|
| Email | `audit.therapist@…` | `audit.admin@…` |
...
**Post-remediation (2026-08-05T12:08Z):** both hashes are now `$2a$10$`, `updated_at` aligned, and Vault holds `vpsych_audit_*_password`. Do not reset again unless Vault verify fails.
...
select u.email,
       (u.encrypted_password = extensions.crypt('<CANDIDATE_PASSWORD>', u.encrypted_password))
         as password_matches
from auth.users u
where u.email in ('audit.therapist@…', 'audit.admin@…')
order by u.email;
```
...
```bash
for email in audit.therapist@… audit.admin@…; do
...
T_TOKEN=$(curl -sS -X POST "$SUPABASE_URL/auth/v1/token?grant_type=password" \
  -H "apikey: $ANON_KEY" -H "Content-Type: application/json" \
  -d "{\"email\":\"audit.therapist@…\",\"password\":\"$VPSYCH_AUDIT_THERAPIST_PASSWORD\"}" \
  | jq -r .access_token)

A_TOKEN=$(curl -sS -X POST "$SUPABASE_URL/auth/v1/token?grant_type=password" \
  -H "apikey: $ANON_KEY" -H "Content-Type: application/json" \
  -d "{\"email\":\"audit.admin@…\",\"password\":\"$VPSYCH_AUDIT_ADMIN_PASSWORD\"}" \
  | jq -r .access_token)
...
4. **Merge the RC3 record to `main`.** It currently exists only in unmerged draft PRs, so it cannot be verified from the declared authoritative source.
5. Consolidate or close the competing RC3 branches (#104, #105, #106).
