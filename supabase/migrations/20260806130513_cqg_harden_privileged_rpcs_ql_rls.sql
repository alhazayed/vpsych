-- CQG Continuous Quality Guardian — privilege hardening
--
-- CQG-001 (Critical): append_quality_ledger was EXECUTE-able by any
-- authenticated user with no is_admin / ownership gate; SECURITY DEFINER
-- bypasses RLS and lets therapists forge the scientific SSOT.
--
-- CQG-002 (High): log_quality_ledger_access / unrestricted audit append
-- pollution of compliance tables via PostgREST.
--
-- CQG-006 (High): "QL learner read own ledgers" exposed assessment scores
-- (payload.scores_overall / VQI fields) to the therapist who sealed the row.
--
-- Also: pin search_path on quality_ledger_reject_mutation (advisor WARN).

-- ─── Quality Ledger append: service_role only (body = prior migration + gate) ─
...
REVOKE ALL ON FUNCTION public.append_quality_ledger(jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.append_quality_ledger(jsonb)
  TO service_role;

COMMENT ON FUNCTION public.append_quality_ledger(jsonb) IS
  'Append-only Quality Ledger seal. SERVICE ROLE only — never grant to authenticated (CQG-001).';

-- ─── Quality ledger access audit: service_role only ─────────────────────────
REVOKE ALL ON FUNCTION public.log_quality_ledger_access(text, uuid, text, jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.log_quality_ledger_access(text, uuid, text, jsonb)
  TO service_role;

COMMENT ON FUNCTION public.log_quality_ledger_access(text, uuid, text, jsonb) IS
  'QL access audit. SERVICE ROLE only (CQG-002).';

-- ─── Security audit: admin or service_role only (block therapist forgery) ───
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role'
    ...
    RAISE EXCEPTION 'log_security_event requires admin or service_role';
  END IF;
...
COMMENT ON FUNCTION public.log_security_event IS
  'Insert a security audit row; actor_id is auth.uid(). Callable by service_role or is_admin() (CQG-002).';

-- ─── Quality Ledger SELECT: admin only (drop learner score leak) ────────────
DROP POLICY IF EXISTS "QL learner read own ledgers" ON public.quality_ledgers;

-- ─── Advisor: mutable search_path on reject trigger ─────────────────────────
...
AS $$
BEGIN
  RAISE EXCEPTION 'quality ledger tables are append-only';
END;
$$;
