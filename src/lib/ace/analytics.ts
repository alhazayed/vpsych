...
export function mapRubricToCompetencies(
  ...
  const bump = (id: CompetencyId, value: number) => {
    const prev = out[id];
    out[id] = prev == null ? value : Math.round((prev + value) / 2);
  };
  ...
  if (out.diagnostic_interview == null) {
    out.diagnostic_interview = Math.round(overall);
  }
  if (out.professional_communication == null) {
    out.professional_communication = Math.round(overall * 0.95);
  }
...
export function applySessionPerformance(
  ...
  // Fill missing competencies with neutral baseline
