...
describe("architecture invariants", () => {
  ...
  it("TTS ignores client-supplied ElevenLabs voice ids", () => {
    const resolve = readFileSync(
      join(root, "lib/voice/resolve-tts-voice.ts"),
      "utf8",
    );
    expect(resolve).toMatch(/Never trust client-supplied ElevenLabs ids/);
    expect(resolve).toMatch(/let legacyVoiceId: string \| null = null/);
  });

  it("mic capture is muted to prevent speaker feedback", () => {
    const rec = readFileSync(join(root, "lib/voice/record-wav.ts"), "utf8");
    expect(rec).toMatch(/mute\.gain\.value = 0/);
