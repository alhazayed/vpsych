...
export function EriDashboardClient() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/eri");
      ...
      if (!res.ok) throw new Error(json.error || "Failed to load ERI");
  ...
  const maxTrend = Math.max(
    1,
    ...(dashboard?.learner_trend.map((t) => t.mean) ?? [1]),
  );
    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-3">
          <h2 className="font-[family-name:var(--font-headline)] text-xl font-bold text-[var(--primary)]">
            Instructor reliability report
          </h2>
          <div className="space-y-2 rounded-lg border border-[var(--outline-variant)] p-4 text-sm">
            <p>
              Mean ERI:{" "}
              <strong>{dashboard?.instructor_report.mean_eri ?? "—"}</strong>
            </p>
            <p>
              Heuristic share:{" "}
              <strong>
                {dashboard?.instructor_report.heuristic_share != null
                  ? `${dashboard.instructor_report.heuristic_share}%`
                  : "—"}
              </strong>
            </p>
            ...
            <ul className="list-disc space-y-1 pl-5">
              {(dashboard?.instructor_report.low_dimension_hotspots ?? []).map(
                (h) => (
                  <li key={h.id}>
      ...
      <section className="space-y-3">
        <h2 className="font-[family-name:var(--font-headline)] text-xl font-bold text-[var(--primary)]">
          Recommendations (low ERI)
        </h2>
        <ul className="list-disc space-y-1 pl-5 text-sm text-[var(--on-surface)]">
          {(dashboard?.low_eri_recommendations ?? []).length === 0 ? (
            <li>No low-ERI recommendations in current corpus.</li>
          ) : (
            dashboard!.low_eri_recommendations.map((r) => (
              <li key={r}>{r}</li>
            ))
          )}
        </ul>
      ...
      <section className="space-y-3">
        <h2 className="font-[family-name:var(--font-headline)] text-xl font-bold text-[var(--primary)]">
          Weight matrix (ERI v{dashboard?.eri_version ?? "1.0.0"})
        </h2>
        <div className="overflow-x-auto rounded-lg border border-[var(--outline-variant)]">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-[var(--surface-container)] text-[10px] uppercase tracking-wider text-[var(--outline)]">
