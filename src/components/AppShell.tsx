...
function therapistNav(labels: {
  patientLibrary: string;
  mySessions: string;
}): NavItem[] {
  return [
    {
      href: "/avatars",
      label: labels.patientLibrary,
      icon: "library_books",
      match: (p) => p.startsWith("/avatars"),
    },
    {
      href: "/sessions",
      label: labels.mySessions,
      icon: "clinical_notes",
...
function adminNav(labels: {
  reportsLibrary: string;
  avatarPresets: string;
}): NavItem[] {
  return [
    {
      href: "/admin/reports",
      label: labels.reportsLibrary,
      icon: "folder_shared",
      match: (p) => p.startsWith("/admin/reports"),
    },
    {
      href: "/admin/avatars",
      label: labels.avatarPresets,
      icon: "psychology",
...
export function AppShell({
  ...
  const nav = [
    ...therapistNav({
      patientLibrary: tNav("patientLibrary"),
      mySessions: tNav("mySessions"),
    }),
    ...(profile.role === "admin"
      ? adminNav({
          reportsLibrary: tNav("reportsLibrary"),
          avatarPresets: tNav("avatarPresets"),
        })
      : []),
  ];
  ...
  const pageTitle = pathname.startsWith("/admin/reports")
    ? tShell("pageTitle.reportsLibrary")
    : pathname.startsWith("/admin/avatars")
      ? tShell("pageTitle.avatarPresets")
      : pathname.startsWith("/sessions")
        ? tShell("pageTitle.mySessions")
        : tShell("pageTitle.patientLibrary");
